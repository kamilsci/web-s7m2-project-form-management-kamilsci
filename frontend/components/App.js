...
// const infoInputs = evt => {
//   const { id, fname, lname, bio } = evt.target
...
export default function App() {
  ...
  const [vals, setVals] = useState(intVals);
  useEffect(() => {
  ...
  const onChange = evt => {
    ...
    setVals(prevVals => ({ ...vals, [id]: value }))
  ...
  const editExistingMember = () => {
    ...
    setMembers(prevMemb => prevMemb.map (mem =>{
      if (mem.id == editing){
        return {...mem, ...vals}
      }
      return mem
      ...
      <div id="membersForm">
        ...
        <form onSubmit={onSubmit}>
          <div>
            ...
            <input onChange={onChange} value={vals.fname} id="fname" type="text" placeholder="Type First Name" />
          </div>

          <div>
            <label htmlFor="lname">Last Name </label>
            <input onChange={onChange} value={vals.lname} id="lname" type="text" placeholder="Type Last Name" />
          </div>

          <div>
            <label htmlFor="bio">Bio </label>
            <textarea onChange={onChange} value={vals.bio} id="bio" placeholder="Type Bio" />
          </div>
